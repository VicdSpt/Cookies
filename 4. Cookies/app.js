...
function handleForm(e) {
  ...
  newCookie.expires = new Date(new Date().getTime() + 7 * 60 * 24 * 60 * 1000);
...
function doesCookieExist(name) {
  ...
  const onlyCookiesNames = cookies.map((cookie) => cookie.split("=")[0]);

  const cookiePresence = onlyCookiesNames.find(
...
function createElements(cookies) {
  cookies.forEach((cookie) => {
    ...
    listItem.innerHTML = `
      ...
      <button>X</button>
    `;
    listItem.querySelector("button").addEventListener("click", e => { createToast({name: name, state: "supprimer", color:"crimson"})
